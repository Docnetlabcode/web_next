...
Setup: `cp .env.example .env.local` and set the backend URL + Firebase keys. If `NEXT_PUBLIC_API_BASE` is blank, `next.config.mjs` rewrites proxy `/api` and `/socket.io` to `BACKEND_PROXY_TARGET` (default `http://localhost:5000`); when set, calls go direct to the backend.
