...
**Dual backend deployment (AWS + Render):** the backend runs in two places. When the `NEXT_PUBLIC_API_BASE_AWS` / `NEXT_PUBLIC_API_BASE_RENDER` pairs are set (see `.env.example`), `src/lib/backend.ts` probes each api-service `GET /health` on load (AWS preferred, Render only when AWS is down), locks onto the first live deployment, and fails over mid-session on network errors — `api.ts` retries the failed request once against the other deployment, and the socket singletons re-point via `onBackendChange`. The legacy single `NEXT_PUBLIC_API_BASE` / `NEXT_PUBLIC_SOCKET_URL` vars still work unchanged when the pairs are unset. Caveat: the httpOnly refresh cookie is scoped to the backend's domain, so login state does not survive a cross-deployment failover.
