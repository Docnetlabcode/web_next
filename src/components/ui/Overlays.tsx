...
export function Modal({ open, onClose, title, children, className }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <div className={cn("relative w-full max-w-lg animate-scale-in rounded-3xl bg-white shadow-2xl", className)}>
        {title && (
...
export function BottomSheet({ open, onClose, title, subtitle, children }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[70] flex items-end justify-center sm:items-center sm:p-4">
      <div className="absolute inset-0 animate-fade-in bg-ink-900/45 backdrop-blur-sm" onClick={onClose} />
      <div className="anim-sheet-up relative w-full max-w-md rounded-t-3xl bg-white pb-3 shadow-2xl sm:rounded-3xl">
...
export function EmojiPicker({ onPick }) {
  return (
    <div className="anim-pop grid w-64 grid-cols-7 gap-1 rounded-2xl border border-ink-900/[.08] bg-white p-2 shadow-card">
      {EMOJI.map((e) => (
